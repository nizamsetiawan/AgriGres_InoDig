...
class KomoditasRekapitulasiFilterSection extends StatelessWidget {
  const KomoditasRekapitulasiFilterSection({super.key});

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<KomoditasRekapitulasiController>();
    ...
    return Column(
      ...
      children: [
        ...
        Obx(() => _buildFilterItem(
          ...
          'Komoditas',
          Text(
            controller.selectedKomoditasName,
            style: textTheme.bodyMedium?.copyWith(color: Colors.grey[800]),
          ),
          onTap: () => _showKomoditasPicker(context, controller),
        )),
        const SizedBox(height: 16),
  ...
  void _showLevelHargaPicker(BuildContext context, KomoditasRekapitulasiController controller) {
  ...
  void _showKomoditasPicker(BuildContext context, KomoditasRekapitulasiController controller) {
    showModalBottomSheet(
      context: context,
      backgroundColor: Colors.transparent,
          ...
          children: [
            ...
            Expanded(
              child: ListView.builder(
                itemCount: controller.komoditasOptions.length,
                itemBuilder: (context, index) {
                  final option = controller.komoditasOptions[index];
                  return Obx(() => ListTile(
                    title: Text(
                      option['name'],
                    ...
                    leading: Radio<int>(
                      value: option['id'],
                      groupValue: controller.selectedKomoditasId.value,
                      onChanged: (value) {
                        if (value != null) {
                          controller.selectKomoditas(value);
                          Get.back();
                        }
                      },
                    ),
